...
interface WizardStep { id: number; name: string; sections: string[]; }
...
const emitField = (label: string, value: string) => {
  window.dispatchEvent(new CustomEvent("contract:wizard-field-change", { detail: { label, value } }));
};

const emitServices = (services: string[]) => {
  window.dispatchEvent(new CustomEvent("contract:wizard-services-change", { detail: { services } }));
};
...
export default function ContractWizard({ wizardStep: externalWizardStep, setWizardStep }: Props) {
  const [step, setStep] = useState(Math.min(Math.max(externalWizardStep || 1, 1), 5));
  const [values, setValues] = useState({
    artist: "", artistEmail: "", client: "", representative: "", email: "", phone: "",
    event: "", date: "", venue: "", duration: "", fee: "", deposit: "50",
    paymentMethod: "", travel: "", cancellation: "", preset: "",
  });
  const [services, setServices] = useState<string[]>([]);
  const [advancedOpen, setAdvancedOpen] = useState(false);

  useEffect(() => {
    setStep(Math.min(Math.max(externalWizardStep || 1, 1), 5));
  }, [externalWizardStep]);

  const update = (key: keyof typeof values, label: string, value: string) => {
    setValues(current => ({ ...current, [key]: value }));
    emitField(label, value);
  };
  ...
  const applyPreset = (label: string) => {
    ...
    emitField("Booking Preset", label);
    emitField("Total Fee", preset.fee);
    emitField("Deposit Percentage (%)", preset.deposit);
    emitField("Performance Duration", preset.duration);
    emitServices(preset.services);
  };
  ...
  return (
    <div className="new-contract-workflow relative z-[100] pointer-events-auto -mx-1 overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm" onPointerDown={event => event.stopPropagation()}>
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="mb-2 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.16em] text-neutral-400">
              <span>New booking</span><span className="h-1 w-1 rounded-full bg-neutral-300" /><span>Step {step} of 5</span>
            </div>
      ...
      <div className="bg-neutral-50/60 px-5 py-6 sm:px-7 sm:py-8">
        {step === 1 && <section className="mx-auto max-w-3xl">
          ...
          <div className="mb-6 rounded-xl border border-neutral-200 bg-white p-4">
            ...
            <select value={values.preset} onChange={e => applyPreset(e.target.value)} className={fieldClass}>
              <option value="">Choose a common booking type</option>{PRESETS.map(preset => <option key={preset.label} value={preset.label}>{preset.label}</option>)}
            </select>
        ...
        {step === 2 && <section className="mx-auto max-w-3xl">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {SERVICES.map(service => {
              ...
              return <button key={service} type="button" aria-pressed={selected} onClick={() => toggleService(service)} className={`flex items-center justify-between rounded-xl border p-4 text-left transition ${selected ? "border-neutral-950 bg-neutral-950 text-white shadow-sm" : "border-neutral-200 bg-white text-neutral-800 hover:border-neutral-400 hover:shadow-sm"}`}><span><span className="block text-sm font-semibold">{service}</span><span className={`mt-1 block text-xs ${selected ? "text-neutral-300" : "text-neutral-400"}`}>{description}</span></span><span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border text-sm font-semibold ${selected ? "border-white bg-white text-neutral-950" : "border-neutral-300 text-neutral-400"}`}>{selected ? "✓" : "+"}</span></button>;
            })}
          </div>
        ...
        {step === 3 && <section className="mx-auto max-w-3xl">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-neutral-200 bg-white p-4"><div className="text-xs font-semibold uppercase tracking-[0.12em] text-neutral-400">Total</div><div className="mt-2 text-xl font-semibold text-neutral-950">{money(values.fee)}</div></div><div className="rounded-xl border border-neutral-200 bg-white p-4"><div className="text-xs font-semibold uppercase tracking-[0.12em] text-neutral-400">Deposit</div><div className="mt-2 text-xl font-semibold text-neutral-950">{money(String(deposit))}</div></div><div className="rounded-xl border border-neutral-200 bg-white p-4"><div className="text-xs font-semibold uppercase tracking-[0.12em] text-neutral-400">Balance</div><div className="mt-2 text-xl font-semibold text-neutral-950">{money(String(balance))}</div></div></div>
        </section>}

        {step === 4 && <section className="mx-auto max-w-3xl">
        ...
        {step === 5 && <section className="mx-auto max-w-3xl">
      ...
      <div className="flex items-center justify-between gap-3 border-t border-neutral-200 bg-white px-5 py-4 sm:px-7">
        ...
        {step < 5 ? <button type="button" onClick={() => goToStep(step + 1)} className="rounded-xl bg-neutral-950 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-neutral-800">Continue</button> : <button type="submit" className="rounded-xl bg-neutral-950 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-neutral-800">Create contract</button>}
      </div>
    </div>
  );
}
