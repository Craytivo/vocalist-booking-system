...
export default function FormPanel({
  ...
  return (
    <section
      className={`rounded-2xl border border-slate-200 bg-white/85 p-4 shadow-xl shadow-slate-200/60 backdrop-blur sm:p-6 lg:w-[380px] lg:overflow-y-auto lg:p-6 lg:sticky lg:top-[64px] lg:h-[calc(100vh-64px)] lg:self-start relative ${
    ...
    >
      <div className="flex flex-col gap-5 sm:gap-6 mb-6">
        {/* Primary Actions */}
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-700 mb-3">Primary Actions</p>
        ...
        {draftId && (
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-700 mb-3">Contract Actions</p>
            <div className="flex flex-wrap items-center gap-2">
              <Button
                variant="secondary"
                size="md"
        ...
        {/* Utilities */}
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-700 mb-3">Utilities</p>
          <div className="flex flex-wrap items-center gap-2">
            <Button
              variant="secondary"
              size="md"
      ...
      <div className="mb-4 rounded-xl border border-slate-200 bg-gradient-to-r from-white/90 to-slate-50 px-4 py-3 flex items-center gap-3 shadow-sm shadow-slate-200/60">
        <span className={`h-2 w-2 rounded-full ${saveStatus.includes("failed") ? "bg-red-500" : saveStatus.includes("Saving") ? "bg-[#6b8f71]" : "bg-emerald-500"}`} />
        <p className="text-sm text-slate-700">{!isOnline ? "Offline - saved locally" : saveStatus}</p>
        {!isOnline && (
      ...
      {/* Progress */}
      <div className="mb-4 rounded-lg border border-slate-200 bg-white/60 px-3 py-2 shadow-sm shadow-slate-200/60">
        <div className="flex items-center justify-between gap-3">
          <span className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-600">Progress</span>
          <span className="text-xs font-semibold text-slate-800">{readinessScore}% ready</span>
        </div>
        {readinessScore < 100 && (
            ...
            {readinessChecks.filter((item) => !item.complete).slice(0, 2).map((item) => (
              <div key={item.field} className="flex items-center justify-between gap-2 text-xs text-neutral-600 dark:text-stone-400">
                ...
                <button
                  ...
                  className="text-slate-700 font-semibold hover:text-slate-900 transition-colors focus-visible:ring-2 focus-visible:ring-slate-300 focus-visible:ring-offset-2 rounded px-1"
                >
                  Add
                </button>
              </div>
      ...
      {/* Search */}
      <div className="mb-4 rounded-xl border border-slate-200 bg-white/80 p-3 shadow-md shadow-slate-200/60">
        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-700 mb-2">Search</p>
        <input
          type="text"
          placeholder="Search contracts..."
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 outline-none transition-all hover:border-slate-300 focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
        />
      </div>
