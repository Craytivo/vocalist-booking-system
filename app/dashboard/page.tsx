...
const stages = [{ id: "Draft", label: "Draft", icon: FileText }, { id: "Ready", label: "Ready", icon: CheckCircle2 }, { id: "Sent", label: "Sent", icon: Send }, { id: "Signed", label: "Signed", icon: FileSignature }];
...
export default function DashboardPage() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur-xl"><div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8"><Link href="/dashboard" className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-950 text-white"><LayoutDashboard size={17} /></span><span><span className="block text-sm font-bold tracking-tight">Setlist</span><span className="block text-[11px] text-slate-500">Booking workspace</span></span></Link><div className="flex items-center gap-2"><Link href="/contract" className="hidden rounded-lg px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 sm:inline-flex">Contracts</Link><Link href="/contract" className="inline-flex items-center gap-2 rounded-lg bg-slate-950 px-3.5 py-2 text-sm font-semibold text-white shadow-sm hover:bg-slate-800"><Plus size={16} /> New contract</Link><button onClick={handleLogout} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700" aria-label="Sign out"><LogOut size={17} /></button></div></div></header>
      <div className="mx-auto max-w-[1440px] px-4 py-8 sm:px-6 lg:px-8">
        <section className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-indigo-600">Overview</p><h1 className="mt-2 text-3xl font-bold tracking-tight sm:text-4xl">Good to see you, {artistName}.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Manage your bookings from inquiry to signed contract without losing track of what needs attention.</p></div><div className="flex items-center gap-2 text-xs text-slate-400"><User size={14} /><span className="max-w-[240px] truncate">{email}</span></div></section>
